...
export class UploadService {
  ...
  makeFileRequest(
    ...
    return new Promise(function (resolve, reject) {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
          //stado que dice que si envio los archivos
          if (xhr.status == 200) {
  ...
  async subirImagen(img: string) {
    ...
    fileTransfer
      .upload(img, this.url + "/subir-cv", options)
      .then(async data => {
  ...
  subirImagenOferta(img: string, id) {
    ...
    fileTransfer
      .upload(img, this.url + `subir-imagen-oferta/${id}`, options)
      .then(data => {
        console.log(data);
      })
      .catch(err => {
  ...
  subirImagenUsuario(img: string, id): boolean {
    ...
    fileTransfer
      .upload(img, this.url + `/subir-imagen-usuario/${id}`, options)
      .then(async data => {
        let dt = JSON.stringify(data.response);
        let token = dt.split('\\"');
        console.log("token: " + token[3]);
